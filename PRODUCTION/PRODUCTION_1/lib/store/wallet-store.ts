...
interface WalletState {
  ...
  // Actions
  createWallet: (mnemonic: string, password: string, ethereumAddress: string, bitcoinAddress: string) => void;
...
export const useWalletStore = create<WalletState>()(
  persist(
    (set, get) => ({
      ...
      createWallet: (mnemonic, password, ethereumAddress, bitcoinAddress) => {
        const wallets: Wallet[] = [
          { address: ethereumAddress, chain: 'ethereum', balance: '0' },
          { address: bitcoinAddress, chain: 'bitcoin', balance: '0' },
        ];
